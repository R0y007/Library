...
async function auth(req,res){const jwt=verifyJwt(cookies(req).avam_session);if(!jwt)return null;const admin=await getAdmin();if(!admin||admin.password_version!==jwt.pv)return null;const csrf=req.headers['x-csrf-token'];if(!csrf||csrf!==jwt.csrf)return null;return jwt;}
...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    const jwt=await auth(req,res);if(!jwt)return fail(res,401,'Authentication required.');

    if(path==='admin/me'&&req.method==='GET')return json(res,200,{authenticated:true,csrfToken:jwt.csrf});
