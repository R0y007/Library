...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    if(path==='admin/change-password'&&req.method==='POST'){
      const b=await body(req);const current=String(b.currentPassword||'');const next=String(b.newPassword||'');if(next.length<14||next.length>200)return fail(res,400,'New password must be 14–200 characters.');const admin=await getAdmin();if(!admin||!verifyPassword(current,admin.password_hash))return fail(res,401,'Current password is incorrect.');await sb(`admins?id=eq.${admin.id}`,{method:'PATCH',body:JSON.stringify({password_hash:hashPassword(next),password_version:admin.password_version+1,updated_at:now()})});clearSession(res);return json(res,200,{ok:true,message:'Password changed. Please sign in again.'});
    }
