...
function pruneRateLimits(nowMs=Date.now()){
  if(rateLimits.size < 500) return;
  for(const [k,v] of rateLimits){
    if(nowMs >= v.reset) rateLimits.delete(k);
  }
  if(rateLimits.size > 5000){
    const entries=[...rateLimits.entries()].sort((a,b)=>a[1].reset-b[1].reset);
    for(let i=0;i<entries.length-4000;i++) rateLimits.delete(entries[i][0]);
  }
}
function rateAllowed(bucket, key, limit, windowMs){
  const t = Date.now();
  pruneRateLimits(t);
  const k = `${bucket}:${key}`;
  let r = rateLimits.get(k);
  if(!r || t >= r.reset){ r = {count:0, reset:t + windowMs}; }
  r.count++;
  rateLimits.set(k,r);
  return r.count <= limit;
}
function clearRateLimit(bucket,key){ rateLimits.delete(`${bucket}:${key}`); }
...
function certInput(x){
  ...
  if(c.id && !/^AVAM-[A-Z0-9]{12}$/.test(c.id))throw Error('Certificate ID is invalid.');
  if(!['Active','Suspended','Revoked'].includes(c.status))throw Error('Invalid certificate status.');
  if(!c.mechName||!c.owner||!validIsoDate(c.dateIssued))throw Error('Name, owner, and a valid issue date are required.');
  for(const [k,max] of Object.entries({combat:15,aesthetic:15,technical:15,admin:5}))if(!Number.isInteger(c[k])||c[k]<0||c[k]>max)throw Error(`Invalid ${k} score.`);
  return c;
}
...
async function publicVerify(req,res,id){
  const ip=clientIp(req);
  if(!rateAllowed('verify',ip,30,10*60*1000)) return fail(res,429,'Too many verification attempts. Please wait a few minutes.');
  const safe=clean(id,40).toUpperCase();
  if(!/^AVAM-[A-Z0-9]{12}$/.test(safe)) return fail(res,400,'Invalid certificate ID.');
  const rows=await sb(`certificates?id=eq.${encodeURIComponent(safe)}&select=*`);if(!rows.length)return fail(res,404,'Certificate ID not found.');return json(res,200,{certificate:publicCert(rows[0])});}
...
module.exports=async(req,res)=>{
  ...
  try{
    if(!SESSION_SECRET || SESSION_SECRET.length < 32)return fail(res,500,'Server is not configured.');
    if(path==='health'&&req.method==='GET')return json(res,200,{ok:true,service:'avam-library'});
    if(path.startsWith('verify/'))return publicVerify(req,res,path.slice(7));

    if(path==='admin/login'&&req.method==='POST'){
      const key=clientIp(req);
      if(!rateAllowed('login',key,8,15*60*1000))return fail(res,429,'Too many login attempts. Please wait 15 minutes.');
      const b=await body(req);const password=String(b.password||'');
      if(password.length<1||password.length>200)return fail(res,400,'Invalid password.');
    ...
    if(path==='admin/bootstrap'&&req.method==='GET'){const jwt=await auth(req,res,false);if(!jwt)return fail(res,401,'Authentication required.');const rows=await sb('certificates?select=*&order=date_issued.desc,id.desc');return json(res,200,{authenticated:true,csrfToken:jwt.csrf,certificates:rows.map(adminCert)});}
    ...
    if(path==='admin/certificates'&&req.method==='POST'){
      const c=certInput(await body(req));
      const t=now();
      let rows=[];
      for(let attempt=0;attempt<5;attempt++){
        c.id=generateCertificateId();
        try{
          rows=await sb('certificates',{method:'POST',body:JSON.stringify({id:c.id,status:c.status,mech_name:c.mechName,owner:c.owner,faction:c.faction,date_issued:c.dateIssued,pvp_types:c.pvpTypes,combat:c.combat,aesthetic:c.aesthetic,technical:c.technical,admin:c.admin,notes:c.notes,created_at:t,updated_at:t})});
          break;
        }catch(e){
          if(!/duplicate|unique/i.test(e.message) || attempt===4) throw e;
        }
      }
      return json(res,201,{certificate:adminCert(rows[0])});
    }
    ...
    if(path==='admin/change-password'&&req.method==='POST'){
      const changeKey=`${clientIp(req)}:${jwt.sub}`;
      if(!rateAllowed('password-change',changeKey,5,15*60*1000))return fail(res,429,'Too many password change attempts. Please wait 15 minutes.');
