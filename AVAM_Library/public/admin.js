...
function setBusy(btn,busy,label){ if(!btn)return; btn.disabled=busy; if(busy){btn.dataset.originalLabel=btn.innerHTML;btn.innerHTML=`<span class="spinner" aria-hidden="true"></span>${label||'Working…'}`;} else if(btn.dataset.originalLabel){btn.innerHTML=btn.dataset.originalLabel;delete btn.dataset.originalLabel;} }
...
async function bootstrap(){
  try{
    // One request for session + registry avoids a visible second loading pause.
    const data = await api('/api/admin/bootstrap');
    csrfToken = data.csrfToken;
    hide(loginView); show(adminView);
    renderRecords(data.certificates || []);
  }catch{
...
$('#loginForm').addEventListener('submit',async(e)=>{
  ...
  const btn=$('#loginForm button[type=submit]'); setBusy(btn,true,'Signing in…');
  try{
...
function renderRecords(certificates){
  const records=$('#records');
  const total=certificates.length; const active=certificates.filter(c=>c.status==='Active').length; const attention=certificates.filter(c=>c.status!=='Active').length;
  $('#recordCount').textContent=total; $('#activeCount').textContent=active; $('#attentionCount').textContent=attention;
  if(!certificates.length){records.innerHTML='<div class="empty">No certificate records on file.</div>';return}
  records.innerHTML=certificates.map(c=>`
    <div class="record">
  ...
  records.querySelectorAll('.edit').forEach(b=>b.addEventListener('click',()=>editRecord(b.dataset.id,certificates)));
  records.querySelectorAll('.delete').forEach(b=>b.addEventListener('click',()=>deleteRecord(b.dataset.id)));
}

async function loadRecords(){
  try{ const data=await api('/api/admin/certificates'); renderRecords(data.certificates || []); }
